 ...
 (defvar my-help-map
  ...
  (defvar my-git-map
    (let ((map (make-sparse-keymap)))
      ...
      (define-key map (kbd "c") #'magit-clone)
      (define-key map (kbd "g") #'magit)
      (define-key map (kbd "l") #'consult-git-log-grep)
      (define-key map (kbd "m") #'git-gutter:next-hunk)
      (define-key map (kbd "s") #'git-gutter:stage-hunk)
      (define-key map (kbd "x") #'git-gutter:revert-hunk)
      (define-key map (kbd "y") #'git-link)
      (define-key map (kbd "z") #'magit-stash-both)
      map))
